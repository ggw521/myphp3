...
<div class="main">
	<p class="short-input ue-clear">
    	<label>用户名：</label>
        <input name="username" value="<?php echo ($data["username"]); ?>" type="text" placeholder="用户名" />
        <input type="hidden" name="id" value="<?php echo ($data["id"]); ?>"/>
    </p>
	<p class="short-input ue-clear">
    	<label>密码：</label>
        <input name="password" type="text" placeholder="密码" />注：不修改则留空
    </p>
    <p class="short-input ue-clear">
    	<label>姓名：</label>
        <input name="truename" value="<?php echo ($data["truename"]); ?>" type="text" placeholder="姓名" />
    </p>
	<p class="short-input ue-clear">
    	<label>昵称：</label>
    ...
    <div class="short-input select ue-clear">
        ...
        <select name="dept_id">
            ...
            <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($val["id"]); ?>" <?php if($data["dept_id"] == $val["id"] ): ?>selected='selected'<?php endif; ?>><?php echo ($val["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
    </div>
	<p class="short-input ue-clear">
    	<label>性别：</label>
